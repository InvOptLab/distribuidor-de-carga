 ...
 */
export class CargaDeTrabalho extends Constraint<null> {
  constructor(
    name: string,
    description: string,
    isHard: boolean,
  ...
  soft(
    ...
    for (const docente of docentes) {
      if (cargaDocente.get(docente.nome) > 2) {
        ...
        // * (1 - cargaDocente.get(docente.nome)); // A condição já me garante que o Carga do docente é < 1, por isso não preciso me preocupar em ocorrerem valores negativos.
      }
    }
