...
export default function NetworkVisualizer({
  ...
  const [networkType, setNetworkType] = useState<"BIPARTITE" | "DOCENTES" | "TURMAS">("BIPARTITE");
  ...
  const graphData = useMemo(() => {
    ...
    // --- 1. PROCESSAMENTO DOS NÓS E LINKS CONFORME PROJEÇÃO ---
    if (networkType === "BIPARTITE") {
      const visibleDocentes = graph.getAllDocentes().filter((d) => isNodeVisible(d.id));
      const visibleTurmas = graph.getAllTurmas().filter((t) => isNodeVisible(t.id));
      ...
      allVisibleNodes.forEach((n) => {
        ...
        if (viewMode === "simulation") {
          if (simulationResult) {
            ...
            color = n.type === NodeType.DOCENTE ? theme.palette.primary.light : theme.palette.secondary.light;
          }
          ...
          const isCritical = report.criticalTeachers.some((c) => c.docenteId === n.id);
          const isLeaf = report.leafNodes.includes(n.id);
          if (n.type === NodeType.DOCENTE) {
            color = isCritical ? theme.palette.error.main : theme.palette.primary.main;
          } else {
            color = isLeaf ? theme.palette.warning.main : theme.palette.secondary.main;
          }
          val = n.type === NodeType.DOCENTE ? graph.getDegree(n.id) : 1;
        }

        nodes.push({ id: n.id, name: n.label, group: n.type, val, color });
      });
      ...
      // PROJEÇÕES (DOCENTES ou TURMAS)
      const projectedGraph = networkType === "DOCENTES" ? graph.projectToDocentes() : graph.projectToTurmas();
      const allNodes = projectedGraph.getAllNodes();
      ...
      allNodes.forEach((n) => {
        nodes.push({
          ...
          name: n.label,
          group: n.type,
          val: projectedGraph.getDegree(n.id),
          color: networkType === "DOCENTES" ? theme.palette.primary.main : theme.palette.secondary.main,
        });
      });

      projectedGraph.edges.forEach((e) => {
        links.push({
          source: e.sourceId,
          target: e.targetId,
          color: theme.palette.divider,
          width: e.weight || 1, // Peso baseado nas turmas/docentes em comum
        });
      });
    }

    return { nodes, links };
  }, [graph, report, theme, viewMode, simulationResult, hiddenCommunities, networkType]);
          ...
          <Box>
            ...
            <Box mt={1}>
              <ToggleButtonGroup
                value={networkType}
                exclusive
                onChange={(e, newType) => newType && setNetworkType(newType)}
                size="small"
                sx={{ mr: 2 }}
              >
                <ToggleButton value="BIPARTITE">Bipartida</ToggleButton>
                <ToggleButton value="DOCENTES">Prj. Docentes</ToggleButton>
                <ToggleButton value="TURMAS">Prj. Turmas</ToggleButton>
              </ToggleButtonGroup>
            </Box>
          </Box>
