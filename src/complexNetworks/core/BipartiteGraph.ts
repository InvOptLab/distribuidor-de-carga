...
export class BipartiteGraph {
  ...
  /**
   * Projeta o grafo bipartido em um grafo unipartido contendo apenas Docentes.
   * Uma aresta é criada entre Docente A e Docente B se eles compartilham pelo menos uma Turma.
   * O peso da aresta será a quantidade de turmas em comum.
   */
  public projectToDocentes(): Graph {
    const projected = new Graph();
    const docentes = this.getAllDocentes();

    // Adiciona todos os nós docentes
    docentes.forEach((d) => projected.addNode(d));

    // Para cada par de docentes, contar turmas em comum
    for (let i = 0; i < docentes.length; i++) {
      for (let j = i + 1; j < docentes.length; j++) {
        const d1 = docentes[i];
        const d2 = docentes[j];

        const turmas1 = new Set(this.getNeighbors(d1.id));
        const turmas2 = this.getNeighbors(d2.id);

        let commonCount = 0;
        turmas2.forEach((t) => {
          if (turmas1.has(t)) commonCount++;
        });
  ...
  /**
   * Projeta o grafo bipartido em um grafo unipartido contendo apenas Turmas.
   * Uma aresta é criada entre Turma A e Turma B se elas podem ser ministradas pelo mesmo Docente.
   */
  public projectToTurmas(): Graph {
    const projected = new Graph();
    const turmas = this.getAllTurmas();

    // Adiciona todos os nós turmas
    turmas.forEach((t) => projected.addNode(t));

    // Para cada par de turmas, contar docentes em comum
    for (let i = 0; i < turmas.length; i++) {
      for (let j = i + 1; j < turmas.length; j++) {
        const t1 = turmas[i];
        const t2 = turmas[j];

        const docentes1 = new Set(this.getNeighbors(t1.id));
        const docentes2 = this.getNeighbors(t2.id);

        let commonCount = 0;
        docentes2.forEach((d) => {
          if (docentes1.has(d)) commonCount++;
        });
